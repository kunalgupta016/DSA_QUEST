...
const GraphDFS = () => {
  ...
  return (
    <div className="min-h-screen bg-gray-900 text-white pt-24 px-8 pb-8 flex flex-col items-center">
      <h1 className="text-4xl font-bold mb-8 text-orange-500">DFS Traversal</h1>
      <div className="mb-8 text-center max-w-3xl space-y-4">
        <p className="text-gray-300">
          <strong>English:</strong> Depth-First Search (DFS) explores as far as
          possible along each branch before backtracking. It uses a Stack (or
          recursion).
        </p>
        <p className="text-gray-400 italic">
          <strong>Hinglish:</strong> DFS har branch mein jitna dur ho sake
          explore karta hai, fir backtrack karta hai. Yeh Stack (ya recursion)
          ka use karta hai.
        </p>
      </div>
          ...
          {Object.keys(nodePositions).map((nodeId) => (
            <motion.div
              ...
              className={`absolute w-14 h-14 rounded-full flex items-center justify-center font-bold text-xl border-4 z-10 transition-colors duration-500
                            ${visited.includes(parseInt(nodeId)) ? "bg-orange-500 border-orange-300 text-black" : "bg-gray-700 border-gray-500 text-white"}
                        `}
              animate={{
