...
const TreeVisualizer = () => {
  console.log("TreeVisualizer mounted");
  ...
  const addValue = () => {
    ...
    const newRoot = insertNode(root ? { ...root } : null, val, 400, 50, 0, 150); // initial x=400 (center of 800px)
    setRoot(newRoot);
    setTreeStructure({ ...newRoot }); // Trigger ID update
    setInputValue("");
  ...
  // Helper to flatten tree for rendering (BFS)
      ...
      <div className="w-[800px] h-[600px] bg-gray-800 border-2 border-gray-600 rounded-lg relative overflow-hidden">
        <svg className="absolute top-0 left-0 w-full h-full pointer-events-none">
          {edges.map((edge, i) => (
            <line
              key={i}
              x1={edge.x1}
              y1={edge.y1 + 20} // center of node approx
              x2={edge.x2}
              y2={edge.y2 + 20}
              stroke="white"
              strokeWidth="2"
        ...
        {nodes.map((node, i) => (
          <motion.div
            ...
            style={{ left: node.x - 20, top: node.y }} // Centering
          >
            {node.value}
          </motion.div>
        ))}
